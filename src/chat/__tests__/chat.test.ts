// Retrieval + chat orchestration tests.
//
...
//
// Coverage:
//  - zero usable hits → no-sources, no Ollama call.
//  - hits with the model citing [1][2], a [7] dropped.
//  - Happy path: streamed answer persists + a reopen renders the sources.
//  - Long history trimmed first to fit num_ctx.
//  - CyborgDB error → RetrievalError, never an ungrounded answer.
//  - Injection: a retrieved chunk with "Ignore previous instructions" /
//    "{{user.name}}" stays literal inside its <source> delimiter.
//  - Snapshot persisted so a reopen renders cards.
...
describe("composePrompt budgeting", () => {
...
describe("composePrompt injection delimiting", () => {
...
describe("runTurn — happy path with citations", () => {
  it("streams an answer, persists with a cited snapshot, drops a dangling ref", async () => {
    ...
    // Persisted + reopen renders the snapshot.
    const convo = await prisma.conversation.findUnique({
      where: { id: conversationId },
      include: { messages: { orderBy: { createdAt: "asc" } } },
    });
...
describe("runTurn — retrieval failure", () => {
...
describe("runTurn — start from spaceId", () => {
...
describe("runTurn — mid-stream drop", () => {
...
describe("buildRetrievalQuery — follow-up context carry", () => {
  ...
  it("folds the last user turns into a follow-up's retrieval query", () => {
    const history = [
      { role: "user", content: "How do I deploy Context Well?" },
      { role: "assistant", content: "Run `context-well up`." },
    ] as const;
    const q = buildRetrievalQuery([...history], "How long does it take?");
    expect(q).toContain("How do I deploy Context Well?");
    expect(q).toContain("How long does it take?");
    // Assistant turns are not folded into the retrieval query.
    expect(q).not.toContain("context-well up");
  });
