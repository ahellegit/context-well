// Citation validation + source-card snapshot.
//
// Inline `[n]` citations map to numbered source cards, and we persist a full
// snapshot of the cited source cards — id, title, snippet, score, connector —
// so a reopened thread renders citations even after a later sync purged the
// underlying vectors.
//
...
// A persisted source card snapshot. Stored as JSON on the Message row.
...
export interface CitationResult {
  ...
  // This is what gets persisted on the assistant message.
  citedCards: SourceCard[];
  // Citation numbers the model emitted that fall outside 1..hits.length and
  // were dropped (diagnostics / flagged).
  droppedNumbers: number[];
}

// Round a 0–1 similarity to 2 decimals for display/storage.
 ...
 * hits, and build the cited source-card snapshot.
 *
