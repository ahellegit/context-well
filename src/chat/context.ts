// Prompt composition + context budgeting for a chat turn.
//
// This composes the custom prompt + retrieved context + query, substituting
// `{{space.name}}` / `{{user.name}}` in the custom-prompt template ONLY — never
// in retrieved hits or history. It respects num_ctx via a fixed trim order:
// system prompt > user query > retrieved context > history; history is trimmed
// first. Retrieved third-party content is wrapped in explicit
// `<source n>…</source n>` delimiters with a system instruction to treat it as
// data, not instructions — a prompt-injection mitigation.
...
// and (capped) context. Ordering: system > query > context > history.
 ...
 * Only `{{space.name}}` and `{{user.name}}` are recognized; any other
...
export interface ComposePromptInput {
  // The raw custom-prompt template for the space (placeholders intact).
  customPrompt: string;
  // Substitution values for the template only.
  vars: { spaceName?: string; userName?: string };
  ...
  // the system message is built here). Trimmed first to fit num_ctx.
  history?: ChatMessage[];
...
// Case-insensitive; the inserted zero-width space keeps the text
...
/** Render a hit as a delimited `<source n>` block. */
 ...
 *
 * Budgeting: the system prompt (custom prompt + data instruction +
 ...
 *
 * Substitution is applied to the custom-prompt template only; hits and
...
export function composePrompt(input: ComposePromptInput): ComposedPrompt {
  ...
  if (grounded) {
    ...
    // large retrieved context cannot crowd out the system prompt/query.
  ...
  // System prompt and user query are non-negotiable (top of the order).
  const fixedTokens =
    estimateTokens(systemMessage.content) + estimateTokens(userMessage.content);

  // History gets the remainder. Trim oldest-first until it fits (history
