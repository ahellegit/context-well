// Connector API plugin. Exposes per-space connector CRUD, validation, target
// listing, and the user-triggered sync. Credential values are NEVER returned to
// the browser — list/read responses mask them to a {set, last4} indicator.
//
 ...
 * set/last-4 indicator. The raw token never crosses the API boundary.
 */
...
export default async function connectorsRoutes(
  ...
  // List a space's connectors (credentials masked). Viewer+ (read).
  ...
  // Add a connector to a space. Stores credentials server-side; the response is
  // masked. Editor+ on the space.
  ...
  // Validate a connector's credentials (the scope check lives in the impl).
  ...
  // Trigger a sync. 409 when a sync already holds the space lock. Editor+ on the
  // connector's space (ingest is a write).
  ...
  // Delete a connector and purge its vectors from the index. Editor+ on the
  // connector's space.
