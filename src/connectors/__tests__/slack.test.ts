// Slack connector tests. `fetch` is fully mocked with a router that
...
describe("slackConnector.sync", () => {
  it("yields chunks for messages + thread replies with full metadata", async () => {
    ...
    // chunk id is the stable hash for (slack, channel, threadRoot, 0).
    expect(thread.id).toBe(chunkId("slack", "C1", "100.000", 0));
    // Thread groups root + replies together, with resolved author names.
