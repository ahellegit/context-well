// Slack connector. Ingests selected public channels' messages + thread replies
// through the shared Connector contract. Built fresh against the Slack Web API
// (https://slack.com/api/) over native fetch — no Slack SDK — with a bot token
// (xoxb-) sent as a Bearer credential.
//
// Grouping: a Slack conversation is chunked PER THREAD (root + replies
...
//
// Security: `validate` checks the token works and warns (does not fail) on
...
// Minimum scopes this connector needs to do its job. Excess scope warns.
 ...
 * if they exceed the minimum this connector needs (least-privilege advisory).
 * Missing required scopes are reported in the warning too.
 */
 ...
 * required scopes (likely to break sync) and excess scopes (advisory, not a
 * failure). Returns undefined when scopes are unknown or exactly minimal.
 */
 ...
 * the bot is invited) — surfaced distinctly, not hidden.
 */
 ...
 * boundaries. The Document ref is the thread root's ts (or the message ts for a
 * standalone), so resync is stable. title is
...
// importing this module (the registry ships empty by design — it does not
