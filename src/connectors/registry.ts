// Connector registry (U7 / R11). Maps a connector `kind` to its implementation
// so the orchestrator and the routes can resolve a connector by kind and so the
// UI can list the installed connectors.
//
// The registry is populated by the connector units (U8 github, U9 slack) via
// `registerConnector` — either through an explicit call those units make or by
// import side-effect. This module ships empty; it does not import the concrete
// connectors itself (that would create a U7→U8/U9 dependency the plan does not
// want). Callers that need a populated registry import the connector modules.
