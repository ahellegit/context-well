// Connector plugin contract (U7). A connector is a self-contained adapter for an
...
// can discover the installed connectors (R11).
//
// Requirements: R11 (shared interface: validate / list targets / sync → chunks +
// metadata), KTD7 (stable chunk IDs), KTD8a (per-source-type chunking — the
// connector decides grouping and calls the chunk.ts helpers).
 ...
 * from KTD7 so resync is idempotent and purgeable.
 *
 ...
 * excess scope on success (R29 — least-privilege advisory, not a hard failure).
 */
...
export interface Connector {
  ...
  /** Check credentials are present and have the minimum required scope (R29). */
