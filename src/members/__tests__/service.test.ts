// Members management tests: account creation, role grant/change/revoke,
...
describe("setRole / revokeRole", () => {
  ...
  it("changing a role downgrades and drops the target's sessions", async () => {
    const target = await makeUser("u@t", "member");
    const space = await makeSpace("a");
    await setRole(actor, target.id, space.id, "editor");
    await createSession(target.id);
  ...
  it("revoke removes the membership, drops sessions, and audits", async () => {
    const target = await makeUser("u@t", "member");
    const space = await makeSpace("a");
    await setRole(actor, target.id, space.id, "viewer");
    await createSession(target.id);
...
describe("promote / demote invariants", () => {
