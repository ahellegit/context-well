// Members & account management API (U4). All routes require a workspace admin
// (plugin-level guard); promote/demote additionally require the owner. Mutations
// are rate-limited (R15). Mounted inside the protected scope by the server, so
// requireAuth + requirePasswordChanged already ran. Implements R3, R4, R5, R9,
// R11, R12, R14, R15.
//
...
// Rate limit admin mutations to bound abuse / account-creation storms (R15).
...
export default async function membersRoutes(app: FastifyInstance): Promise<void> {
  ...
  // POST /api/members — create an account with a temp password (R9). Returns the
  ...
  // POST /api/members/:userId/roles — grant or change a per-space role (R4).
  ...
  // DELETE /api/members/:userId/roles/:spaceId — revoke a per-space role (R4, R8).
  ...
  // POST /api/members/:userId/promote — make a member an admin (R3). Owner only.
  app.post(
  ...
  // POST /api/members/:userId/demote — demote an admin to member (R3, R5). Owner only.
  app.post(
  ...
  // POST /api/members/:userId/reset-password — issue a fresh temp password (R11).
