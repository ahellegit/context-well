// Members & account management service. Admin-driven account creation,
...
// admin reset (verifyLogin rejects an expired temp password).
 ...
 * change. No session is created (no auto-login). The temp password is
...
/**
 * Grant or change a user's per-space role. Upserts the Membership; on a
...
/**
 * Revoke a user's membership of a space. Deletes the row (no-op if
...
/** Promote a member to admin. Route restricts the caller to the owner. */
...
/**
 * Demote an admin to member. The owner can never be demoted, and the
...
export async function demoteAdmin(actorUserId: string, targetUserId: string): Promise<void> {
  ...
  // Defense in depth: never leave zero root holders. With an undemotable
 ...
 * Drops their sessions and returns the temp password once. Memberships
