...
// streaming chat call relaying Ollama's NDJSON. The connection test does a
// GET / liveness check plus GET /api/tags discovery, filtering chat models from
// embedding models by details.family; the SSRF guard vets the URL; and the
// streaming path supports cancellation with a server-side stream timeout.
...
export async function testConnection(
  ...
  try {
    ...
    // metadata host the SSRF guard already vetted away.
 ...
 * caller's AbortSignal and a server-side inactivity timeout backstop.
 *
...
export async function* streamChat(
  ...
  try {
    ...
    // already vetted away — refuse to follow it.
    if (res.status >= 300 && res.status < 400) {
