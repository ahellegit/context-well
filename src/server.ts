...
export async function buildServer() {
  ...
  // Protected scope: everything registered here requires a valid session.
  await app.register(async (protectedScope) => {
    registerAuthGuard(protectedScope);
    await protectedScope.register(settingsRoutes); // absolute /api/settings paths
    await protectedScope.register(spacesRoutes); // absolute /api/spaces paths
    await protectedScope.register(connectorsRoutes);
    await protectedScope.register(uploadsRoutes); // file uploads
    await protectedScope.register(chatRoutes);
    await protectedScope.register(membersRoutes); // RBAC members management
  });
