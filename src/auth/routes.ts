...
export default async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  // "sign in" without a user-facing register toggle.
  app.get("/bootstrap-status", async () => {
    return { needsBootstrap: await isFirstAccount() };
  });

  // POST /register — bootstrap-only. The very first account creates the
  ...
  // POST /login — verify credentials and rotate the session id (rotate on login).
  ...
  // POST /change-password — rotate the caller's password. Lives in the
