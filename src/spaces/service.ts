// Knowledge space service. Owns Space CRUD (provisioning/tearing down the
// CyborgDB index alongside the DB row), the raw custom-prompt template, and the
// per-space conversation/message history.
//
// A space owns its prompt, index, and conversations: creating one provisions an
// index and deleting one removes it, conversations persist per space with
// messages, sources, and updated time, and the custom prompt is stored raw with
// `{{var}}` placeholders whose substitution happens at chat time.
//
...
// down before the row so a failed teardown never orphans the index.
...
// A Space without its secret 32-byte index key. Used everywhere a
// Space crosses the API boundary so the key never reaches the browser.
 ...
 * (namespace-as-slug), so it must be unique workspace-wide.
 */
 ...
 * provisionIndex mints + persists the index key on the row when it has no
...
/**
 * Spaces visible to a caller: every space for a workspace owner/admin, and
 ...
 * The connector sync lock is not consulted here; that concern lives
...
/**
 * Store the raw custom-prompt template for a space. The `{{var}}`
 * placeholders are kept verbatim; substitution is deferred to chat time.
 */
 ...
 * Conversations are private (per-user chat privacy): scoped to `userId`.
 */
...
export interface AppendMessageInput {
  ...
  text: string;
  // Full snapshot of cited source cards; serialized to JSON for storage.
  sources?: unknown[];
}

/**
 * Append a message to a conversation and touch the conversation's `updatedAt`
 * so the thread floats to the top of the list. The two writes run in a
 * transaction so the timestamp and the message stay consistent. The `sources`
 * snapshot is stored as JSON text.
 */
